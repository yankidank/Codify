...
export const addJob = async (jobProperties) => {
	...
	try {
		const { companyName: displayName, position, state, city, remote, salary, url, notes, status } = jobProperties;
		let newCompany = await axiosInstance.post('/api/companies', {displayName});
		let newJob = await axiosInstance.post('/api/jobs', {company: newCompany.data._id, post : {url, position, city, state, remote, salary, notes}, status});
