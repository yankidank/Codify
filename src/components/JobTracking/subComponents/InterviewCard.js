...
function InterviewCard(props) {
  const {
    handleInputChange, time, addNewInterview, date, remote, street, city, state, notes, index, _id
  } = props;
    ...
    <div className="card card-padded card-position">
      <div className="interviewInputs">
        ...
        <div className="input-field col s12">
          <input name="street" value={street} id={`interview-street-${_id}`} className="validate" type="text" onChange={(event) => handleInputChange(event, index, _id)} />
          <label htmlFor={`interview-street-${_id}`} className={street ? 'active' : ''}>Street Address</label>
        </div>
        <div className="input-field col s8">
          <input name="city" value={city} id={`interview-city-${_id}`} className="validate" type="text" onChange={(event) => handleInputChange(event, index, _id)} />
          <label htmlFor={`interview-city-${_id}`} className={city ? 'active' : ''}>City</label>
        </div>
        <div className="input-field col s4">
          <input name="state" value={state} id={`interview-state-${_id}`} className="validate" type="text" onChange={(event) => handleInputChange(event, index, _id)} />
          <label htmlFor={`interview-state-${_id}`} className={state ? 'active' : ''}>State</label>
        </div>
        <div className="input-field col s12">
          <textarea name="notes" value={notes || ''} id={`interview-notes-${_id}`} className="materialize-textarea" type="text" onChange={(event) => handleInputChange(event, index, _id)} />
